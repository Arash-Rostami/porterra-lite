...
export default function ProductField({ value, onChange, onCategorySelect }) {
  ...
  function submit() {
    ...
    if (!trimmed || !category) { toast('نام محصول و دسته‌بندی الزامی است'); return; }
    if (products.some((p) => p.name.toLowerCase() === trimmed.toLowerCase())) { toast('این محصول قبلاً ثبت شده'); return; }
    addProduct({ id: 'PROD-' + Date.now() + '-' + Math.floor(Math.random() * 10000), name: trimmed, category, isCustom: true, createdAt: Date.now() });
    onChange(trimmed);
  ...
  return (
    <div style={{ display: 'flex', gap: 6, alignItems: 'stretch' }}>
      <Dropdown value={value} onChange={onChange} options={options} placeholder="انتخاب کنید" />
      ...
      >
        <div className="crm-form-grid">
          ...
          <div className="crm-field -span2">
            ...
            <Dropdown value={category} onChange={setCategory} options={CATEGORY_OPTS} placeholder="انتخاب کنید" />
          </div>
        </div>
      </Modal>
    </div>
