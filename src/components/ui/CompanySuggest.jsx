...
export default function CompanySuggest({ records, value, onChange, onSelect, onBlur, ...inputProps }) {
  const [open, setOpen] = useState(false);
  const blurTimer = useRef(null);

  const q = Utils.normText(value);
  let matches = [];
  if (q) {
    const companyCounts = {};
    for (const r of records) {
      const c = Utils.normSpace(r.company);
      if (!c || Utils.normText(r.company).indexOf(q) === -1) continue;
