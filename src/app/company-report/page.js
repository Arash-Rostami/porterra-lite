...
export default function CompanyReportPage() {
    const { records, reminders } = useScopedData();
    const searchParams = useSearchParams();
    const initialCompany = searchParams.get('company') || '';

    return (
        <div className="crm-tab-panel" id="crmPanelCompany">
            <CompanyReport records={records} reminders={reminders} onMarkReminderDone={markReminderDone} onOpenRecord={openProfile} initialCompany={initialCompany} />
        </div>
    );
}
