  ...
  if (!isElevated(user)) {
    ...
    if (scope.type === 'own') {
      if (!rec.coordinator) rec.coordinator = scope.agentCode;
        ...
        const err = new Error('VALIDATION: نمی‌توانید سرنخی برای این کارشناس ثبت کنید') as Error & { code?: string };
        err.code = 'VALIDATION';
        throw err;
      }
    } else if (scope.type === 'department') {
      if (!rec.coordinator || !scope.agentCodes.includes(rec.coordinator)) {
        const err = new Error('VALIDATION: این کارشناس در دپارتمان شما نیست') as Error & { code?: string };
        err.code = 'VALIDATION';
        throw err;
      }
    }
